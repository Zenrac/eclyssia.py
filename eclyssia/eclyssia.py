...
log = logging.getLogger('eclyssia')


class Client:
    def __init__(self, token: str = '', bot=None, loop=None, aiosession=None):
        self._headers = {
            "User-Agent": "Eclyssia.py (GitHub: Zenrac)",
            "Authorization": token
        }
    ...
    def pluggable(cls, bot, token: str = "", *args, **kwargs):
        ...
        if hasattr(bot, 'eclyssia'):
            return bot.eclyssia
        bot.eclyssia = cls(token, bot=bot, *args, **kwargs)
        return bot.eclyssia
    ...
    async def get_endpoints(self, bypass=False):
        ...
        try:
            ...
            log.info('Session is closed... Can\'t fetch information from eclyssia.')
